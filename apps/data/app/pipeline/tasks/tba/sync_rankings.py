...
def sync_rankings(event_filter: EventFilter = "all", current_year: bool = False):
    logger = get_run_logger()

    logger.info(
        f"Starting rankings sync from The Blue Alliance (event_filter={event_filter}, current_year={current_year})"
    )
    ...
    logger.info(
        f"Rankings sync completed successfully (event_filter={event_filter}, current_year={current_year})"
    )
